...
public class NativeInteractionActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button button = (Button)findViewById(R.id.btn_ni_back);
    ...
    private void showAd(final TTNativeAd ad, boolean isExpress) {
        //是模板类型渲染，可以直接调用 ad.showInteractionExpressAd(this)展示广告
        //非模板类型，则按原逻辑，自行原生渲染并展示
        if (isExpress){
            //绑定dislike逻辑，有助于精准投放
            bindDislikeAction(ad, false);
            //绑定广告view事件交互
    ...
    private void bindDislikeAction(TTNativeAd ad, boolean isCustomDislike) {
        if (isCustomDislike) {
            // 使用自定义Dislike
            ...
            if (ttAdDislike != null) {
                ...
                ttAdDislike.setDislikeInteractionCallback(new TTAdDislike.DislikeInteractionCallback() {
                    ...
                    public void onSelected(int position, String value, boolean enforce) {
                        TToast.show(mContext, "\t\t\t\t\t\t\t感谢您的反馈!\t\t\t\t\t\t\n我们将为您带来更优质的广告体验", 3);
                        if (enforce) {
                            TToast.show(mContext, "NativeInteractionActivity 原生插屏 sdk强制移除View ");
                        }
                    }
