...
//    private TTAdSlot mExpressAdSlot; //用于本地构建动态布局
    ...
    public void setData(TTFeedAdOnePointFiveImpl feedAd, List<TTFeedAd> ads) {
        if (ads == null || ads.size() == 0) {
            return;
        }
        mTTFeedAdOnePointFive = feedAd;
        mFeedAdList.clear();
    ...
    protected void onAttachedToWindow() {
        ...
        if (mAdInteractionListener != null) {
            ...
            mAdInteractionListener.onAdShow(mTTFeedAdOnePointFive);
        }
    }
    ...
    private void updateNextOnePointFiveAd() {
        if (mNeedUpdate.getAndSet(false)) {
            ...
            mPreloadAdList = null;
            mTTFeedAdOnePointFive.mFeedAdList = mFeedAdList;
            //mCommerceType = getLastMetaCommerceType();
            render();
